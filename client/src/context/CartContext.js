...
export const CartProvider = ({ children }) => {
  ...
  const addToCart = async (foodItemId, quantity) => {
    try {
      const res = await axios.post('/api/users/cart', { foodItemId, quantity });
      setCart(res.data);
      setError(null);
      return res.data;
    } catch (err) {
  ...
  const updateCartItem = async (itemId, quantity) => {
    try {
      const res = await axios.put(`/api/users/cart/${itemId}`, { quantity });
      setCart(res.data);
      setError(null);
      return res.data;
    } catch (err) {
  ...
  const removeFromCart = async (itemId) => {
    try {
      const res = await axios.delete(`/api/users/cart/${itemId}`);
      setCart(res.data);
      setError(null);
      return res.data;
    } catch (err) {
